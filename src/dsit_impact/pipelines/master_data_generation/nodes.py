...
sectional data, and diversity scores. The main function, `create_master_dataset`, 
merges various dataframes to create a comprehensive base dataset.

Functions:
    create_master_dataset(gtr_to_oa_map, oa, s2_papers, s2_citations, section_details, ...):
...
Usage:
    ...
    The `create_master_dataset` function is the main entry point, which takes 
...
def create_master_dataset(
    ...
    master_data = oa.merge(s2_papers, on="id", how="left")
    aggregated_s2_citations = _aggregate_citations(s2_citations)
    master_data = master_data.merge(aggregated_s2_citations, on="id", how="left")
    ...
    for column in [
        ...
        master_data.fillna({column: 0}, inplace=True)
        master_data[column] = master_data[column].astype(int)

    logger.info("Merging PDF sectional data.")
    section_details = _aggregate_citation_sections(section_details)
    master_data = master_data.merge(section_details, on="id", how="left")
    master_data.fillna({"total_sections": 0}, inplace=True)
    master_data["total_sections"] = master_data["total_sections"].astype(int)
    master_data["section_counts"] = master_data["section_counts"].apply(
        lambda x: x if isinstance(x, list) else []
    )

    logger.info("Merging diversity scores.")
    master_data = master_data.merge(coauthor_diversity, on="id", how="left")
    ...
    master_data = master_data.merge(paper_diversity, on="id", how="left")
    master_data.rename(
        columns={
    ...
    master_data = master_data[master_data["outcome_id"].notnull()]
